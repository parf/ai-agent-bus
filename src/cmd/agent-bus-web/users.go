...
<section class="editor-card compact-card"><h2>Identity</h2><div class=detail-meta>{{$identity := identityLabel .User .RecordKinds}}{{$authority := authorityLabel .User.DaemonOwner .User.Administrator}}{{with $identity}}<span class=fact-pill>{{.}}</span>{{end}}{{if ne $identity $authority}}<span class=fact-pill>{{$authority}}</span>{{end}}</div><h2>Groups</h2><div class=choice-row>{{range .User.Groups}}<a class=group-chip href="/group?name={{.}}">{{.}}</a>{{else}}<span class=muted>No memberships</span>{{end}}</div></section>
