...
{{if or .New .User.CanEdit}}<section class=editor-card aria-labelledby=profile-heading><h2 id=profile-heading>Profile</h2><form id="form-{{if .New}}create{{else}}save{{end}}" method=post action=/user><input type=hidden name=return value="{{.Return}}"><div class=form-grid>
