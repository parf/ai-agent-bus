...
func TestPagesDoNotPromiseWhatTheDaemonRefuses(t *testing.T) {
	...
	marks := map[string]string{
		...
		"/agents": "<a href=/agents aria-current=page>", "/personal": "<a href=/agents aria-current=page>", "/queues": "<a href=/queues aria-current=page>", "/pubsub": "<a href=/pubsub aria-current=page>", "/users": "<a href=/users aria-current=page>",
	...
	for _, path := range []string{"/", "/agents", "/services", "/personal", "/queues", "/pubsub", "/users", "/groups", "/activity", "/diagnostics"} {
		body := get(path)
