...
<label class="form-field form-field-wide"><span class=field-heading>Allow list <button type=button class=help-button popovertarget=create-access-help aria-label="About initial access" data-tooltip="One identity per line. Empty is owner and Maintainers only; * shares with every admitted principal. Personal services may list services only.">ⓘ</button></span><textarea name=allow rows=5 placeholder="service@realm&#10;@group&#10;*">{{.Form.Value "allow"}}</textarea><small>One name or <code>*</code> per line.</small></label></div>
<div popover id=create-access-help class=context-help><h2>Initial access</h2><ul><li>Empty allows only the owner and assigned Maintainers. Add names or * to share.</li>{{if not .Channels}}<li>A Personal service may name only other registered services directly; users, groups, <code>*</code>, itself and Maintainers are refused.</li>{{end}}</ul></div>
...
<label class="form-field form-field-wide"><span class=field-heading>Master access <button type=button class=help-button popovertarget=master-help aria-label="About master access" data-tooltip="This record may refuse configured master access. The daemon Owner's management authority is separate.">ⓘ</button></span><span class=choice-row><span><input type=checkbox name=no_master {{if $.Form.Is "save"}}{{if $.Form.Checked "no_master"}}checked{{end}}{{else}}{{if .NoMaster}}checked{{end}}{{end}}> Refuse configured master access</span></span></label>
{{if .Personal}}<p class="muted form-field-wide">Personal classification, Allow and Maintainers are changed together below.</p>{{else}}<input type=hidden name=edit_allow value=1><label class="form-field form-field-wide"><span class=field-heading>Allow list <button type=button class=help-button popovertarget=settings-access-help aria-label="About the allow list" data-tooltip="One identity per line. Empty is owner and Maintainers only; * shares with every admitted principal.">ⓘ</button></span><textarea name=allow rows=5>{{if $.Form.Is "save"}}{{$.Form.Value "allow"}}{{else}}{{join .Allow "\n"}}{{end}}</textarea><small>One name or <code>*</code> per line.</small></label>{{end}}</div>
...
<div class=page-title><h1>{{titleMark "groups"}} Groups</h1><button type=button class=help-button popovertarget=groups-help aria-label="About group membership" data-tooltip="Groups are reusable authority lists. Enter one identity or nested group per line; the protected administrators group always retains the daemon owner.">ⓘ</button></div><div popover id=groups-help class=context-help><h2>Group membership</h2><ul><li>Enter one identity or nested group per line.</li><li>Administrators may edit ordinary groups; only the daemon owner changes <code>@administrators</code>.</li><li>The daemon owner remains a direct member of <code>@administrators</code>.</li></ul></div>` + formErrorSummary + `<nav class=section-nav aria-label="Group views">{{range .SectionLinks}}{{if .Current}}<a href="{{.Href}}" aria-current=true>{{else}}<a href="{{.Href}}">{{end}}{{.Label}}{{if .Counted}} ({{.Count}}){{end}}</a>{{end}}</nav><div class=group-grid>{{range $name,$members := .Groups}}<section class="editor-card group-card"><div class=page-title><h2><span role=img aria-label="Group">{{groupGlyph}}</span> <code>{{$name}}</code></h2>{{if eq $name "@administrators"}}<span class=fact-pill>protected</span>{{end}}</div>{{if and $.Administrator (or $.DaemonOwner (ne $name "@administrators"))}}<form id="form-group-{{$name}}" method=post action=/groups><input type=hidden name=name value="{{$name}}"><label class=form-field>Members<textarea name=members rows=6 placeholder="user@realm&#10;@nested-group" aria-invalid="{{if and ($.Form.Matches "save" "name" $name) ($.Form.Invalid "members")}}true{{else}}false{{end}}" aria-describedby="{{if and ($.Form.Matches "save" "name" $name) ($.Form.Invalid "members")}}group-error{{end}}">{{if $.Form.Matches "save" "name" $name}}{{$.Form.Value "members"}}{{else}}{{join $members "\n"}}{{end}}</textarea><small>One identity or group per line.</small></label>{{if $.Form.Matches "save" "name" $name}}<p class=warn id=group-error>{{$.Form.Error}}</p>{{end}}<div class=form-actions><button name=action value=save>Save members</button></div></form>{{end}}</section>{{else}}<p>No groups registered.</p>{{end}}</div>
