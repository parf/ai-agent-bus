...
<h2>Remove registration</h2><form method=post action=/service><input type=hidden name=name value="{{.Name}}"><input type=hidden name=action value=delete><p>Drain the queue and stop readers first. The service credential goes with the address, so whatever held this name can no longer call as it. A person's own credential stays: it is not a record's to drop.</p><button>Remove idle service</button></form>
...
<h1>Groups</h1>{{range $name,$members := .Groups}}<h2>{{$name}}</h2>{{if and $.Administrator (or $.DaemonOwner (ne $name "@maintainers"))}}<form method=post action=/groups><input type=hidden name=name value="{{$name}}"><label>Members <input name=members value="{{join $members " "}}"></label><button name=action value=save>Save members</button>{{if ne $name "@maintainers"}}<button name=action value=delete>Delete unused group</button>{{end}}</form>{{if eq $name "@maintainers"}}<p class=muted>This group cannot be deleted, and the daemon owner stays in it.</p>{{end}}{{end}}{{else}}<p>No groups registered.</p>{{end}}
