...
{{with .Record}}<h1>{{.Name}}{{if .Personal}} <span class=muted>· Personal</span>{{end}}</h1><p>Type: <strong>{{entityLabel .Kind}}</strong> · Owner: {{.Owner}}{{with .Maintainers}} · Maintainers: {{.}}{{end}}{{if .Mode}} · Delivery: {{if eq .Mode "pubsub"}}a copy to each subscriber{{else}}one at a time{{end}}{{end}}</p>
