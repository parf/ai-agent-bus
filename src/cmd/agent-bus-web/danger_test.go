...
func TestDangerZoneUsesFreshConfirmationAndResourceReturnPaths(t *testing.T) {
	...
	visible, _ := p.request("bob@h", "GET", "/service?name=%23view%40h", nil, http.StatusOK)
	...
	refused, _ := p.request("bob@h", "GET", "/service-danger?name=%23view%40h", nil, http.StatusForbidden)
	...
	for _, name := range []string{"#svc@h", "jobs@h"} {
		...
		for _, heading := range []string{"<h2>Transfer ownership</h2>", "<h2>Remove registration</h2>"} {
			if !strings.Contains(danger, heading) {
				t.Fatalf("%s Danger Zone omitted %q", name, heading)
			}
		}
		// A configuration is an agent's, a service's or a group's; a queue
		// holds none, so its Danger Zone offers no replacement.
		if configurable := name != "jobs@h"; strings.Contains(danger, "<h2>Replace configuration</h2>") != configurable {
			t.Fatalf("%s Danger Zone offers configuration: %v, want %v", name, !configurable, configurable)
		}
		detailPath := "/agent"
		if name == "jobs@h" {
			detailPath = "/channel"
		...
		if strings.Count(danger, `action=/service-confirm`) != 2 || strings.Count(danger, `action=/service>`) != configForms {
	...
	// A configuration sent to a queue anyway is the daemon's to refuse.
