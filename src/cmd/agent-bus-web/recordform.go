...
{{if holdsSecret $kind}}<div popover id=form-secret-help class=context-help><h2>The {{if eq $kind "agent"}}agent{{else}}service{{end}} secret</h2><ul><li>Opaque bytes. The daemon stores what it is sent and does not parse it; <code>KEY=value</code> is a convention between callers.</li><li>{{if eq $kind "agent"}}The agent itself{{else}}Whoever the allow list admits{{end}} reads the bytes back with <code>agent-bus secret &lt;name&gt;</code>. Every listing and record answer carries only its digest.</li><li>This field is never filled in again, here or anywhere else &mdash; not after a refusal and not on the record&rsquo;s own page.</li></ul></div>{{end}}{{end}}`
...
// A user's own record is the user's inbox, and takes these three and nothing
// else from the kind table (docs/constitution.md#common-record-fields).
func queuePolicy(kind string) bool {
	return kind == protocol.KindAgent || kind == protocol.KindQueue || kind == protocol.KindUser
}

// holdsSecret is the kinds whose record form asks for a secret: an agent and a
// service. A group holds one too and asks for it on its own form; a queue, a
// topic and a user's inbox hold none (docs/constitution.md#-private-values).
func holdsSecret(kind string) bool {
	return kind == protocol.KindAgent || kind == protocol.KindService
}
