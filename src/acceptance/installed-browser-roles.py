...
def direct_service_save(page, name: str, description: str):
    with page.expect_navigation() as navigation:
        page.evaluate(
            """([name, description]) => {
              const values = {
                action: 'save', name, descr: description, addr: '', protocol: '',
                ttl: '', overflow: 'strict', bound: '0', edit_allow: '1', allow: 'dave@fresh'
              };
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        bob.get_by_label("Name", exact=True).fill("browser-svc@fresh")
        bob.get_by_label("Description").fill("Browser matrix service")
        bob.get_by_label("Allow", exact=False).fill("dave@fresh")
        submit(bob, "Register")
        check("browser-svc@fresh" in bob.locator("h1").inner_text(), "resource Owner did not register its service")
        settings = bob.locator("#form-save")
        settings.locator('input[name="descr"]').fill("Resource Owner updated")
        submit(bob, "Save settings")
        check(bob.locator('input[name="descr"]').input_value() == "Resource Owner updated", "resource Owner settings did not persist")
        sharing = bob.locator("#form-personal")
        sharing.locator('textarea[name="allow"]').fill("dave@fresh")
        sharing.locator('textarea[name="maintainers"]').fill("carol@fresh")
        submit(bob, "Save classification and sharing")
        check("Maintainers: carol@fresh" in bob.locator("body").inner_text(), "resource Owner did not assign the Maintainer")
        bob.goto(args.base + "/channels/new", wait_until="domcontentloaded")
        bob.get_by_label("Name", exact=True).fill("browser-topic@fresh")
        bob.get_by_label("Description").fill("Browser matrix channel")
        bob.get_by_label("Queue").check()
        bob.get_by_label("Allow", exact=False).fill("dave@fresh")
        submit(bob, "Register")
        check("one at a time" in bob.locator("body").inner_text(), "resource Owner did not register the queue channel")
        bob.screenshot(path=args.evidence / "browser-resource-owner.png", full_page=True)
        bob_context.close()
        results["resource_owner"] = ["register service", "edit settings", "assign maintainer", "register queue channel"]
        ...
        check(maint.locator("#form-save").count() == 1, "assigned Maintainer lacks settings")
        check(maint.locator("#form-personal").count() == 0, "Maintainer received owner-only classification")
        maint.locator('#form-save input[name="descr"]').fill("Maintainer updated")
        submit(maint, "Save settings")
        check(maint.locator('input[name="descr"]').input_value() == "Maintainer updated", "Maintainer settings did not persist")
        ...
        maint_context.close()
        results["maintainer"] = ["edit settings", "no classification", "no transfer"]
        ...
        admin.goto(args.base + "/groups/new", wait_until="domcontentloaded")
        admin.get_by_label("Name", exact=True).fill("@browser-team")
        admin.get_by_label("Members", exact=False).fill("dave@fresh")
        submit(admin, "Register group")
        team = admin.locator('[id="form-group-@browser-team"]')
        team.locator("textarea").fill("dave@fresh\ncarol@fresh")
        team.get_by_role("button", name="Save members").click()
        admin.wait_for_load_state("domcontentloaded")
        check(admin.locator('[id="form-group-@administrators"]').count() == 0, "Administrator can edit the protected group")
        ...
        check(admin.locator("#form-save").count() == 0, "Administrator can edit the daemon Owner")
        ...
        admin_context.close()
        results["administrator"] = ["create user", "create/edit ordinary group", "protected authority denied", "unrelated service denied", "foreign origin denied"]
        ...
        for path in ("/users/new", "/groups/new"):
            response = ordinary.goto(args.base + path, wait_until="domcontentloaded")
            check(response.status == 403 and "Not yours to see" in ordinary.locator("h1").inner_text(), f"ordinary caller was not refused at {path}")
        ordinary_context.close()
        results["ordinary"] = ["read-only visible service", "service mutation denied", "user/group administration denied"]
        ...
        owner.goto(args.base + "/groups", wait_until="domcontentloaded")
        check(owner.locator('[id="form-group-@administrators"]').count() == 1, "daemon Owner cannot edit the protected group")
        protected = owner.locator('[id="form-group-@administrators"]')
        protected.locator("textarea").fill("owner@fresh\nalice@fresh")
        protected.get_by_role("button", name="Save members").click()
        owner.wait_for_load_state("domcontentloaded")
        team = owner.locator('[id="form-group-@browser-team"]')
        team.locator("textarea").fill("dave@fresh\ncarol@fresh\nbob@fresh")
        team.get_by_role("button", name="Save members").click()
        owner.wait_for_load_state("domcontentloaded")
        ...
        owner.goto(args.base + "/service?name=fresh-echo%40fresh", wait_until="domcontentloaded")
        ...
        check(owner.locator("svg.activity-chart").count() == 1, "real installed service activity never produced a graph")
        ...
        owner_context.close()
        results["daemon_owner"] = ["create user", "edit protected and ordinary groups", "manage another owner's service", "real activity graph"]
