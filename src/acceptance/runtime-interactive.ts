...
// bun runtime-interactive.ts BUILT_PROGRAM_DIR NEW_EVIDENCE_DIR codex|opencode|claude [OTHER_ACCOUNT]
//
// Claude runs against the same kind of loopback model under a literal dummy
// key. Channels are unavailable to such a profile (claude-channel-live.ts owns
// that exchange), so its kickoff is typed and its service answer is collected
// with a filtered ab_consume, the MCP minimum's other documented path.
...
if (runtime !== "codex" && runtime !== "opencode" && runtime !== "claude") throw new Error("expected codex, opencode or claude");
...
const errors: string[] = [];
const screen = (o: string) => o.replace(/\x1b\[[0-9;?<>=]*[a-zA-Z~]/g, "").replace(/\x1b\][^\x07\x1b]*(\x07|\x1b\\)/g, "").replace(/\x1b[()][0-9A-Za-z]/g, "").replace(/\s+/g, "");
const shows = (s: { output: string }, text: string) => s.output.includes(text) || screen(s.output).includes(text.replace(/\s+/g, ""));
const provider = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(req) {
  const path = new URL(req.url).pathname;
  const slot = Number(path.split("/")[1]);
  if (runtime === "claude" && path.endsWith("/count_tokens")) return Response.json({ input_tokens: 10 });
  if (runtime === "claude" && !path.endsWith("/messages")) return new Response("fixture route unavailable", { status: 404 });
...
try {
  ...
  for (const s of slots) {
    ...
    const claudeConfig = join(s.home, "claude");
    if (runtime === "claude") {
      mkdirSync(claudeConfig, { mode: 0o700 });
      // Only first-run onboarding, folder trust and approval of this literal dummy key.
      writeFileSync(join(claudeConfig, ".claude.json"), JSON.stringify({ hasCompletedOnboarding: true, theme: "dark", customApiKeyResponses: { approved: ["fixture-only"], rejected: [] }, projects: { [s.cwd]: { hasTrustDialogAccepted: true } } }));
    } else if (runtime === "codex") writeFileSync(join(s.home, "config.toml"), `model="fixture"\nmodel_provider="fixture"\n[model_providers.fixture]\nname="fixture"\nbase_url="http://127.0.0.1:${provider.port}/${s.slot}/v1"\nwire_api="responses"\nrequires_openai_auth=false\n[projects.${JSON.stringify(s.cwd)}]\ntrust_level="trusted"\n`);
    else {
    ...
    s.proc = Bun.spawn([join(bin, `ab-${runtime}`)], {
      cwd: s.cwd, env: { ...ownerEnv, HOME: s.home, CODEX_HOME: s.home, XDG_CONFIG_HOME: join(s.home, "config"), XDG_DATA_HOME: join(s.home, "data"), OPENCODE_DISABLE_MODELS_FETCH: "1", XDG_STATE_HOME: join(s.home, "state"), XDG_CACHE_HOME: join(s.home, "cache"), AGENT_BUS_NAME: s.name,
        ...(runtime === "claude" ? { CLAUDE_CONFIG_DIR: claudeConfig, ANTHROPIC_BASE_URL: `http://127.0.0.1:${provider.port}/${s.slot}`, ANTHROPIC_API_KEY: "fixture-only", CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC: "1" } : {}) },
  ...
  await until(async () => {
    // Claude asks once about development channels; take its local-development default.
    for (const s of slots) if (runtime === "claude" && !confirmed.has(s.slot) && shows(s, "I am using this for local development")) { confirmed.add(s.slot); setTimeout(() => s.proc!.terminal!.write("\r"), 500); }
    return slots.every(s => runtime === "claude" ? shows(s, "for shortcuts") : s.output.includes(runtime === "codex" ? "Ask Codex" : "commands")) && (await owner.ls()).filter(r => slots.some(s => s.name === r.name) && r.reading).length === 2;
  }, "both native TUIs and pushers ready", 60000);
  if (runtime === "claude") await Bun.sleep(3000);
  for (const s of slots) s.proc!.terminal!.write(`KEYBOARD-${s.slot}`);
  await until(() => slots.every(s => shows(s, `KEYBOARD-${s.slot}`)), "keyboard text rendered");
  ...
  await until(() => slots.every(s => shows(s, `KEYBOARD-OK-${s.slot}`)), "both native TUIs answer keyboard input", 30000);
  ...
  for (const s of slots) {
    ...
    if (runtime !== "claude") await peers[s.slot]!.send({ to: s.name, topic: "interactive", tag: `slot-${s.slot}`, body: `KICKOFF-${s.slot}` });
  }
  if (runtime === "claude") {
    for (const s of slots) s.proc!.terminal!.write(`KICKOFF-${s.slot}`);
    await until(() => slots.every(s => shows(s, `KICKOFF-${s.slot}`)), "kickoff text rendered");
    await Bun.sleep(300);
    for (const s of slots) s.proc!.terminal!.write("\r");
  }
  ...
  for (const s of slots) {
    ...
    if (runtime !== "claude") { // Claude runs no network sidecar of its own
  ...
  await until(() => slots.every(s => s.model.serviceSent && (runtime === "claude" || shows(s, `REQUEST-SENT-${s.slot}`))), "real MCP listing, refusal and service sends");
  // Claude's filtered wait must be standing before the answer arrives, or the
  // face's inbox reader takes it for a channel this profile cannot show.
  if (runtime === "claude") await until(() => slots.every(s => s.model.consumeAt && Date.now() - s.model.consumeAt > 2000), "filtered ab_consume standing");
  ...
  await until(() => slots.every(s => s.model.replied && shows(s, `COMPLETE-${s.slot}: ${replies.get(s.slot)}`)), "correlated replies execute through MCP and appear in native TUIs", 60000);
  for (const s of slots) {
    ...
    check(!screen(s.output).includes(replies.get(sibling.slot)!) && !s.output.includes(replies.get(sibling.slot)!) && !s.model.requests.some(b => JSON.stringify(b.input ?? b.messages ?? "").includes(replies.get(sibling.slot)!)), `session ${s.slot} never receives its sibling's answer`);
