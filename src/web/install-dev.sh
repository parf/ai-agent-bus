...
id agent-bus-web >/dev/null 2>&1 || useradd --system --no-create-home --home-dir /nonexistent --shell /usr/sbin/nologin agent-bus-web
mkdir -p /var/lib/agent-bus
if [ -e "$link" ] && [ ! -L "$link" ]; then echo "$link exists and is not a link; not touching it" >&2; exit 1; fi
ln -sfn "$here" "$link"
install -m 0644 "$here/agent-bus-web.service" "$unit"
