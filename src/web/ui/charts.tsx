...
export function DayChart({ slots, id, compact }: { slots: Slot[]; id: string; compact?: boolean }) {
  const live = SERIES.filter(s => total(slots, s.key) > 0);
  const payload = JSON.stringify({ t: slots.map(s => Math.floor(new Date(s.at).getTime() / 1000)), series: live.map(s => ({ key: s.key, label: s.label, values: slots.map(x => x[s.key] ?? 0) })) });
  ...
  return <figure class={`daychart ${compact ? "compact" : ""}`}>
    <div class="uplot-host" id={id} data-chart={payload} role="img"
      aria-label={`Activity per ten-minute slot${start ? ` from ${slotLabel(start)} to ${slotLabel(end!)}` : ""}; shared maximum ${max} over the displayed nonzero series`}></div>
    <figcaption>
      <ul class="legend">{live.map(s => <li class={`lg-${s.key}`}><span class="swatch" aria-hidden="true"></span>{s.label}: <strong>{number(total(slots, s.key))}</strong> in the last day</li>)}</ul>
    </figcaption>
  </figure>;
}
