...
func TestAnUnknownNameCannotActAndCannotBeIssuedACredential(t *testing.T) {
	...
	for _, probe := range []struct{ method, path, body string }{
		...
		{"POST", "/manage", `{"name":"admin@h","disabled":true}`},
		{"POST", "/unregister", `{"name":"admin@h"}`},
		{"POST", "/register", `{"name":"theirs@h"}`},
		{"POST", "/register", `{"name":"ghost@h"}`},
	} {
	...
	if code, body := call("admin@h", "POST", "/token", `{"name":"nobody@h"}`); code != 401 {
	...
	if code, body := call("admin@h", "POST", "/register", `{"name":"ghost@h"}`); code != 200 {
		t.Fatalf("the owner could not register the name: %d %s", code, body)
	}
	if code, body := call("admin@h", "POST", "/token", `{"name":"ghost@h"}`); code != 200 {
	...
	if code, body := call("ghost@h", "POST", "/register", `{"name":"theirs@h"}`); code != 200 {
...
func TestEveryPrivateRouteRefusesAnUnregisteredName(t *testing.T) {
	...
	for _, route := range privateRoutes {
		...
		for _, how := range []struct {
			...
			body := `{"name":"ghost@h","config":{},"create":true}`
			r := httptest.NewRequest(method, path, strings.NewReader(body))
			w := httptest.NewRecorder()
			if how.credential == "" {
