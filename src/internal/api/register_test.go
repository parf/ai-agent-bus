...
func TestConditionalRegistrationClaimsNameOnce(t *testing.T) {
	...
	for i := 0; i < cap(results); i++ {
		...
		go func(i int) {
			defer wg.Done()
			r := httptest.NewRequest("POST", "/register", strings.NewReader(fmt.Sprintf(`{"name":"Session@h","descr":"claim %d"}`, i)))
	...
	r := httptest.NewRequest("POST", "/register", strings.NewReader(`{"name":"session@h","descr":"renamed"}`))
	r.Header.Set(HeaderToken, credential)
	w := httptest.NewRecorder()
	h.ServeHTTP(w, r)
