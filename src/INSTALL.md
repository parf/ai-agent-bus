...
- Linux with systemd 254 or newer and unified cgroup v2.
- `bwrap` (bubblewrap) and working unprivileged user namespaces.
- Root once, through `sudo`, for installation.
- Bun and a supported Claude Code, Codex or OpenCode CLI only when using that
  runtime's `ab-*` launcher. They are not required by the daemon or dashboard.
...
state under `/var/lib/agent-bus`, and `agent-busd.service` under systemd.
