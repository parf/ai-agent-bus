...
| A service is external; what runs on this bus is an agent | [five record kinds](03-services-and-topics.md#five-record-kinds) | 2026-09-18 owner decision; the word was doing two jobs, and the one it keeps is the thing a registration only describes. Current [how to call it](03-services-and-topics.md#how-to-call-it) still reads the pre-enum way until the plan lands |
| Personal is a flag on an agent, not a kind | [five record kinds](03-services-and-topics.md#five-record-kinds); current [personal and shared](03-services-and-topics.md#personal-and-shared) still describes the generic era | 2026-09-18 owner decision; the classification was never a kind and the field already exists beside one |
...
| 2026-09-18 | The call counts move from the shared footer to the signed-in Overview node strip, and the node strip labels its record count `Services + Agents + Channels` | Owner instruction: put the node's numbers where the owner reads them, in the middle of the landing page, and name what the count actually sums — it is `len(records)`, every kind, so `Records` invited a comparison with the Services and Channels pages that would never agree. It narrows where the dashboard shows the counts, not what the daemon publishes: `GET /identity` still answers them to any caller with no credential, so the 2026-09-16 closed list stands and only its placement is revised | [what a node says about itself](05-discovery.md#what-a-node-says-about-itself), [overview](05-discovery.md#overview-and-diagnostics) |
