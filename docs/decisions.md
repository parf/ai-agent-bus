...
| 2026-09-18 | A record of kind `agent` is an **Inbox**, listed with the channels rather than the services; supersedes the `👾 Agent` label | Owner instruction, from reading `/services` on a node whose every record is an agent: *they are no services*. An agent record carries no `addr` and no `protocol`, so nothing is served from it &mdash; [messaging](04-messaging.md#inbox-queues) already defines it as an implicit queue topic named after the agent, and the page contradicted the document. The label names the record rather than the principal: the principal is an agent, the record is the queue it reads. The stored kind is unchanged, so this is a display and page-placement decision and no registration, ACL or wire value moves | [service kinds](03-services-and-topics.md#service-kinds), [inbox queues](04-messaging.md#inbox-queues), [display labels](05-discovery.md#identity-labels-in-web-and-cli) |
