...
|---|---|---|
| A service secret is opaque bytes (Q77) | [services § secrets](06-services.md#secrets) | 2026-09-19 owner decision; `KEY=value` is a caller convention, and the daemon stores and returns the bytes without parsing — the promise [configuration](03-records.md#configuring-a-template) already makes. Checking would oblige us to define blank lines, comments, `export`, duplicate keys and invalid identifiers, none of which the daemon has a use for |
...
| 2026-09-19 | The Overview node strip breaks after the counts, so Uptime leads the second row rather than the whole strip | Owner instruction. What the node holds is the reason to look at the strip; how long it has been up is context for it. The break is explicit rather than left to the viewport, or which row a cell lands on would depend on the window | [overview](05-discovery.md#overview-and-diagnostics) |
