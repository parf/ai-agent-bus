...
| 2026-09-16 | The dashboard ships no colour-scheme or density control; settles the control half of Q62 | Both schemes ship, but the choice between them is the operating system's: `light-dark()` and a container query do the whole job from the token set. A control is not a widget — it is a preferences route, a cookie, a persistence scope and a decision about whether a preference follows the person or the browser, and MVP stores no preference at all. Cheap to add later, and not free now. Who owns the token file remains open | [theme and density controls](../Plans/MVP/web/visual-design.md#theme-and-density-controls) |
