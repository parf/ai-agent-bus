...
| checked again at **every publish** | **Pending for 0.7:** an inactive recipient refuses the whole publication with an error rather than being skipped ([constitution](constitution.md#common-record-fields)). Through 0.6: that the recipient still exists, is on the bus and is **active**. A suspended user takes no copies, and neither does a name that has since been unregistered. Neither counts as a drop: there is nothing it was entitled to take |
