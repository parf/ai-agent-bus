...
| **One exception, owner decision 2026-09-23: the anonymous page's project picture comes from the project's repository** | A stranger should see what this is, and the picture lives where the project does. It is the only external asset anywhere, it is on the one page nobody has yet signed in to, and it fails to a missing image rather than to a broken page. Every page behind the gate still fetches nothing from anywhere else, and the visitor's request reaching GitHub is the known cost of it |
