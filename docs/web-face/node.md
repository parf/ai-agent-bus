...
| Older doc | Code |
|---|---|
| `Plans/MVP/web/pages.md#landing-and-sign-in-`: acquisition help mentions the SSH onboarding path | the popover states two commands and the non-expiry note only |
...
| Older doc | Code |
|---|---|
| `Plans/MVP/web/pages.md#overview-`: heading has observation time and a Refresh | no time or Refresh on the page; the footer states `Generated` |
...
| `Plans/MVP/web/glyphs.md#where-a-glyph-is-allowed`: one severity glyph per attention item | no glyph; the level is a coloured left border only |
...
| Older doc | Code |
|---|---|
| `Plans/MVP/web/pages.md#activity-activity`: about 145 readings; a restart empties history; `¿` marks unobserved slots | 144 fixed slots, saved across restarts; down time is `0`; no `¿` |
...
| Older doc | Code |
|---|---|
| `Plans/MVP/web/pages.md#diagnostics-diagnostics`: sections are exchanges, refusals and losses | also inboxes holding messages and leftovers; order is refusals, inboxes, exchanges, loss, leftovers |
