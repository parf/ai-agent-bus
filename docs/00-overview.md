...
- **Token from your SSH key**: `export AGENT_BUS_USER_TOKEN=$(ssh agent-bus@localhost token)`; setup puts the pubkey into `agent-bus`'s `authorized_keys` with a forced command, the daemon returns a token bound to that principal → `02`, `04`
