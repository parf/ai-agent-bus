...
| **`kv` in the catalogue** | already listed, and its surface is written down: values with a ttl, `cas`, hashes, and lists with blocking forms and pull-push ([bundled services § data](../13-bundled-services.md#data)). `cas` and pull-push each claim work without a lock, which is most of the state want |
