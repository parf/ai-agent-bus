...
| Part | Proposed |
|---|---|
| Certificate | **self-signed**, generated by `agent-bus-setup` for the node (owner, 2026-09-25); no CA, no DNS name, nothing bought. Asking for TLS and missing the certificate or key refuses the start, as the web face already does ([where it listens](../../docs/05-discovery.md#where-it-listens)) |
| Trust | **a pinned fingerprint**: a client trusts exactly the certificate whose SHA-256 fingerprint it was given, delivered with the token by `ssh agent-busd@<node> token`, the channel that already authenticates the node |
| Clients | the CLI, the MCP face, the launchers, the runner and the web face accept an `https://` daemon address and check the pinned fingerprint; a mismatch refuses the call, never falls back to plain HTTP |
...
Settled 2026-09-25 (Q131, Q132): self-signed with a pinned fingerprint is
enough for agents talking to the daemon between the owner's own hosts.
Renewing the certificate changes the fingerprint, so clients fetch it again
the same way.
