...
| Q68 | What a runner does with an `orig-to` it cannot serve — no child for that name, or the child is gone | owner, with the runner | [runner § dispatch is keyed on orig-to](runner.md#dispatch-is-keyed-on-orig-to-not-on-what-the-runner-owns). Routine rather than exceptional, since being routed a name you do not manage is deliberate. Refusing to the sender, answering with an error, holding it until something can serve it, or discarding it are four different promises — and holding is the one that quietly fills the inbox every other routed name shares |
