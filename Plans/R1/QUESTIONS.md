...
| Q64 | Whether a reply to a routed message comes from the addressed name or from the route target | owner, with the runner | [runner § many names into one inbox](runner.md#many-names-into-one-inbox). The sender asked `scaler@h` and a reply from `runner@h` is a different `from` than they addressed. The runner is handed child credentials and could answer as the child — it may not mint one ([what the child is told](runner.md#what-the-child-is-told)) — so both are reachable, and matching is on topic and tag either way |
| Q67 | Whether a route's permission is checked only when the field is written, or again at delivery — and what a route whose permission has lapsed does | owner, with the runner | [runner § a route may only name a queue you could send to](runner.md#a-route-may-only-name-a-queue-you-could-send-to). Write-time alone means a route outlives the standing that justified it: set it while permitted, and it keeps delivering after the target tightens its ACL or the router is paused. Re-checking at delivery closes that and opens a second question — a message that may no longer be routed is refused to its sender, or falls back to the addressed record's own inbox, and those are different promises |
| Q66 | Whether routing applies to pub/sub subscribers, and whether a routed subscriber collapses its copies | owner, with the runner | [runner § many names into one inbox](runner.md#many-names-into-one-inbox). `fanout` puts a copy in each subscriber's own inbox, which is a different delivery path from a direct send. If routed subscribers are honoured, a reader subscribed under several routed names receives several copies in one inbox, distinguished only by `orig-to` |
