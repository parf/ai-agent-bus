...
| Q65 | Whether `route` may be changed after registration, and what becomes of messages already delivered under the old one | owner, with the runner | [runner § many names into one inbox](runner.md#many-names-into-one-inbox). Registration is where the owner put it. A route that can never change means a runner cannot hand a service over without re-registering it; one that can means delivered messages sit in a queue the record no longer names, and nothing moves them |
