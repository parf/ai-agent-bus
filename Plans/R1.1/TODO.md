...
| A full inbox answers `429` | Declared record state, which is what frees `503` | A send to a bounded queue at its limit is refused `429` while still counted as `full`; the dashboard and refusal totals are unchanged. Leave it answering `503` and a caller cannot tell it from a service that is restarting, which is the check that must fail |
