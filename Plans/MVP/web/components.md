...
| Line-list textarea | One plain identity, group member or ACL term per line; submitted lines survive validation and each refused line receives its own error | The same component edits group members, ACLs and the accepted Maintainers list; ACL additionally admits `*` and runtime `@owner`. Glyphs never enter editable syntax |
