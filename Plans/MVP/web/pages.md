...
| **A removed record vanishes retroactively** | `total()` iterates the **current** `b.records` for every sample, so a removed record leaves the whole series at once — past buckets included. Its traffic was in yesterday's picture and is not in today's, with nothing marking the change. The registry went 21 → 4 on one restart, so this is not hypothetical. **It does not produce a negative delta or a clamped zero**: `prev` and `next` are computed over the same population, so the aggregate simply steps down. I attributed the `max(0, …)` clamp first to restarts and then to removals; neither holds, and the clamp needs no explanation here — codex |
