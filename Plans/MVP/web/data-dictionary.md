...
| `Reading` | an **unfiltered** read is outstanding on this inbox right now, so the next message would be taken. A read restricted to a topic or tag is excluded (core/bus.go `withLiveness`) — and not because it takes nothing: `deliver` serves a matching one of those *ahead* of an unfiltered reader, so the bit is about which reads are represented | **reader attached** / **no unfiltered reader**, everywhere — the dashboard, the MCP listing and anything else that renders the bit — each with a note saying which readers the column leaves out | "Serving"/"Offline". A busy process between pulls is not offline, and this is not health — and *no reader waiting*, which is false while a filtered reader is attached. The [accepted Readers count](../../../docs/05-discovery.md#readers) includes all waits and is pending implementation |
