...
| F.13.4 | Users, groups and account journeys; [directory classification and cleanup built](done/identity-cleanup.md#scope) | [F.13.1](done/web-meanings.md#scope); F.13.2; add persistent/wire support for the [planned GitHub profile fields](web/data-dictionary.md#github-profile--planned) | Mixed directory fixture exceeds a page and includes long names, missing profiles, unclassified identities, a complete GitHub profile and one with absent optional fields. Find a person, inspect memberships/resources and exercise lifecycle under daemon Owner/Administrator/ordinary-user authority. Import GitHub name and public email into blank existing fields; an email collision leaves Email blank without refusing enrolment. Render company, location, Twitter/X handle, avatar URL and Gravatar ID from the trusted daemon answer; create no second email field and never hotlink the avatar. Inspect own fingerprints; retain hidden-membership distinction and protected-group behavior. Remove paging/filter retention, label hidden membership empty, offer protected deletion, expose another user's credentials, overwrite non-empty Person name or Email, admit a duplicate email, refuse enrolment because optional email collided, add a duplicate GitHub-email field or add a remote image request separately: each fails. Requests to the bus per directory page must not grow with rendered avatar count; restore per-avatar full-directory fetching and that check fails |
