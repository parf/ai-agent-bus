...
| **unblocked, in progress** | F.4–F.6, the dashboard the review settled ([discovery § what it shows](../../docs/05-discovery.md#what-it-shows)). Most of the work is in the daemon keeping what a view needs, not in the page — and it now keeps what five of the eight views need. Exchanges, my names and people are still unbuilt, and so is every page |
...
| F.6 | the views the MVP owes | registry, stuck inboxes, exchanges, my names, loss by name, refusals, node, people. Each row in [discovery § what it shows](../../docs/05-discovery.md#what-it-shows) names what the daemon must start keeping — that is the work, not the page. ⚠️ *the daemon now keeps what five of them need*: registry was already `/ls`, and stuck inboxes, loss by name, node and refusals are fed by `oldest`, per-inbox `dropped`/`expired`, the unclean-restart fact ([discovery § what a listing answers](../../docs/05-discovery.md#what-a-listing-answers)) and a count per reason ([discovery § refusals](../../docs/05-discovery.md#refusals)). Exchanges, my names and people are still unbuilt, and so is every page |
