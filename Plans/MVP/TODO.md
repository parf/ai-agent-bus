...
| H.11 | Restore validation gaps found by the 0.7 STABLE verification | 0.7 K.2, K.10 | Restore ignores and reports a stored group naming a nested group that does not exist, and an account mapping whose referent is no live actor; a record ignored for a duplicate ID keeps its credential as an incorrect record does; the daemon account's socket follows an ownership transfer without a restart. Each has a failing check. Known upgrade hazard, not a row: a 0.7.<20 development database whose user record carries a list is ignored with its User's records |
| F.14 | Web refusal truthfulness found by the 0.7 pre-STABLE review | F.13.2 | A problem page shows the daemon's message, not its raw `{"error":…}` envelope, and names a suspension as such rather than "Not yours to see"; the session cookie follows the idle-based session lifetime instead of a fixed 30 minutes; presence checks assert the element, not a bare name. Each has a failing check |
