...
Next close H.9.5 (cross-account runtime sidecar isolation). H.5.3 has [administrative crash-recovery evidence](done/administrative-durability.md#checks). H.5.2 has [real-SSH evidence](done/ssh-onboarding.md#checks); G.1.3 has [installed web-isolation evidence](done/web-isolation.md#checks). The [review evidence](done/release-gap-review.md#findings) distinguishes reproduced failures from unverified risks. Then complete upgrade/recovery, browser and live-runtime acceptance alongside the existing packaging and local-account work. Choices are tracked in [QUESTIONS](QUESTIONS.md#open-questions).
...
Local mapping administration is the operator key or a user token ([administering the account map](../../docs/09-setup.md#administering-the-account-map)); it is not hidden inside a new Settings page. G.1.2 resource limits remain a confinement dependency for release. Read-only screenshots do not close F.12. UI development can use an isolated populated daemon while installed isolation work proceeds separately.
