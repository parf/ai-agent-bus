...
| F.12 | [Installed browser acceptance](../../docs/05-discovery.md#browser-acceptance); [session/restart foundation complete](done/installed-browser-foundation.md#checks) | [H.1 complete](done/fresh-install.md#checks); required dashboard tabs built; F.13.6 for the redesigned journeys after F.13.0 | In a real browser on the installed host, exercise service/channel, user and group controls as daemon Owner, Administrator, resource Owner/Maintainer and ordinary user; check denial paths and activity graphs. Retain the built sign-in/out, cookie and separate web/bus restart checks. Break form-origin validation or authorization separately: each fails its check |
