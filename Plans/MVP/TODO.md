...
| F.13.3 | Service and channel journeys; [compact Services search/kind/sort and detail built](done/web-registry-redesign.md#checks); [reader filter and pagination built](done/web-registry-pagination.md#checks); [compact registration and management forms built](done/web-administration-redesign.md#checks); [Danger Zone built](done/service-danger-zone.md#checks); [section navigation and registration built](done/web-section-navigation.md#checks); [row emphasis and compact time built](done/service-row-emphasis.md#checks); [caller-visible owner photo built](done/github-profile.md#checks) | [F.13.1](done/web-meanings.md#scope); F.13.2; [built Maintainers-list model](done/maintainers-list.md#checks) | Complete populated visitor/owner journeys for both service and channel modes. Preserve invalid ACL and Maintainer lines and never emit glyphs into syntax. Use populated pub/sub and queue fixtures and return to the affected detail. Retain the built search, kind/delivery/reader filters, sort, pagination with URL state, descriptions, navigation, registration pages, ownership marks, Danger Zone, fresh confirmation, resource-specific returns and owner-photo visibility boundary. Hide summaries behind edit permission, accept a Maintainer wildcard, conflate delivery modes, expose a dangerous form on ordinary detail or redirect a channel edit to Services separately: each fails |
| F.13.4 | Users, groups and account journeys; [directory classification and cleanup built](done/identity-cleanup.md#scope); [compact User administration built](done/web-administration-redesign.md#checks); [Group table/detail and provider-optional GitHub login built](done/group-detail-and-github-availability.md#checks); [default public-profile adapter built](done/default-github-profile-adapter.md#checks); [registration entry pages built](done/web-section-navigation.md#checks); [GitHub metadata and local photo built](done/github-profile.md#checks) | [F.13.1](done/web-meanings.md#scope); F.13.2 | Complete the mixed-directory and role journeys with long names, missing profiles, unclassified identities and absent optional provider fields. Find a person, inspect memberships/resources and exercise lifecycle under daemon Owner/Administrator/ordinary-user authority. Retain valid unique GitHub-login updates when optional provider metadata is unavailable; explicit Refresh remains atomic on failure. Retain fill-blank imports, bounded local photo/Gravatar/initials fallback, one directory answer and no remote image. Inspect own fingerprints; retain linked Group detail, hidden-membership distinction and protected-group behavior. Remove paging/filter retention, label hidden membership empty, offer protected deletion, expose another user's credentials, overwrite non-empty Person name or Email, admit a duplicate email, fetch on unrelated edit, partially commit a failed explicit refresh, add a duplicate GitHub-email field, add a remote image request or restore per-avatar daemon fetching separately: each fails |
