...
|---|---|---|---|
| G.4 | Remove master access | Owner decision 2026-09-18; [ACL](../../docs/02-access.md#acl) | The second ACL layer goes: the `-master` flag, `Bus.Masters`, the `masters` map, the `NoMaster` record field and the web checkbox. `may()` grants only through ownership, resource management and the allow list. The owner's [node-wide management](../../docs/01-identity-and-roles.md#daemon-owner) is unaffected, and the two node-wide observability sites (`recent.go`, `activity.go`) keep answering the owner under that authority rather than under master. A record with a non-empty allow list not naming the owner refuses the owner's **send**, while the owner can still manage and see it. Restore any one of the flag, the map entry, the field or the grant line: a named check fails. A restored snapshot carrying `no_master` loads without error and ignores it |
...
| F.13.3 | Service and channel journeys; [compact Services search/kind/sort and detail built](done/web-registry-redesign.md#checks); [compact registration and management forms built](done/web-administration-redesign.md#checks); [Danger Zone built](done/service-danger-zone.md#checks); [section navigation and registration built](done/web-section-navigation.md#checks); [row emphasis and compact time built](done/service-row-emphasis.md#checks); [caller-visible owner photo built](done/github-profile.md#checks) | [F.13.1](done/web-meanings.md#scope); F.13.2; [built Maintainers-list model](done/maintainers-list.md#checks) | Add the remaining independent reader-state filter and pagination with URL state retained, then complete populated visitor/owner journeys for both service and channel modes. Preserve invalid ACL and Maintainer lines and never emit glyphs into syntax. Use populated pub/sub and queue fixtures and return to the affected detail. Retain the built search, kind/delivery filters, sort, descriptions, navigation, registration pages, ownership marks, Danger Zone, fresh confirmation, resource-specific returns and owner-photo visibility boundary. Hide summaries behind edit permission, accept a Maintainer wildcard, conflate delivery modes, expose a dangerous form on ordinary detail or redirect a channel edit to Services separately: each fails |
| F.13.4 | Users, groups and account journeys; [directory classification and cleanup built](done/identity-cleanup.md#scope); [compact User and Group administration built](done/web-administration-redesign.md#checks); [registration entry pages built](done/web-section-navigation.md#checks); [GitHub metadata and local photo built](done/github-profile.md#checks) | [F.13.1](done/web-meanings.md#scope); F.13.2 | Complete the mixed-directory and role journeys with long names, missing profiles, unclassified identities and absent optional provider fields. Find a person, inspect memberships/resources and exercise lifecycle under daemon Owner/Administrator/ordinary-user authority. Retain GitHub setup/change/refresh atomicity, fill-blank imports, bounded local photo/Gravatar/initials fallback, one directory answer and no remote image. Inspect own fingerprints; retain hidden-membership distinction and protected-group behavior. Remove paging/filter retention, label hidden membership empty, offer protected deletion, expose another user's credentials, overwrite non-empty Person name or Email, admit a duplicate email, fetch on unrelated edit, partially commit a failed profile update, add a duplicate GitHub-email field, add a remote image request or restore per-avatar daemon fetching separately: each fails |
| F.13.5 | Overview, activity and envelope diagnostics; [receipt correlation built](done/exchange-evidence.md#checks); [resource activity built](done/service-activity.md#checks); [compact diagnostics presentation built](done/web-administration-redesign.md#checks) | [F.13.1](done/web-meanings.md#scope); F.13.2–F.13.3 | Complete the Overview and remaining diagnostics journeys. Retain the built graph values, real-time positions, shared scale, partial interval, restart-empty history and zero-versus-absent distinction. Exercise reused topic/tag across participants, receipt references, redirected replies and a missing original request; never assert completion without evidence. Merge unrelated exchanges, label dequeued completed, or restore uncontrolled refresh separately: each fails. Keep MVP bounded diagnostics, not the R1 explorer |
