...
First close the shared-host safety gaps: G.1.3 (web authority), H.5.3 (administrative crash recovery), and H.9.5 (runtime sidecars). H.5.2 has [real-SSH evidence](done/ssh-onboarding.md#checks). The [review evidence](done/release-gap-review.md#findings) distinguishes reproduced failures from unverified risks. Then complete upgrade/recovery, browser and live-runtime acceptance alongside the existing packaging and local-account work. Choices are tracked in [QUESTIONS](QUESTIONS.md#open-questions).
