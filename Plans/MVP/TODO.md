...
| F.13.3 | Service and channel journeys | [F.13.1](done/web-meanings.md#scope); F.13.2; accepted Maintainers-list model | Search by description/address, independently filter availability/reader state, sort and page with state retained, then inspect owner/access/queue information as a read-only visitor. Show a caller-visible User owner's local photo without provider or directory-wide fetching. Edit ACL and Maintainers through the same one-term-per-line textarea pattern, preserving invalid lines and never emitting glyphs into syntax. Put Replace configuration, Transfer ownership and Remove registration only behind the red Danger Zone link; retain fresh confirmation for the latter two. Use populated pub/sub and queue fixtures, edit each with authorized credentials, and return to the affected detail. A protected resource and its owner photo remain hidden. Remove descriptions, hide summaries behind edit permission, accept a Maintainer wildcard, conflate delivery modes, expose a dangerous form on ordinary detail, fetch a provider image from the page or redirect a channel edit to Services separately: each fails |
...
| Maintainers list | Replace the single Maintainers-group field with an owner-assigned list of named users, groups, agents and services. Resolve groups through ordinary nested membership; preserve owner-only replacement and existing management checks. Reject glyphs and wildcard, round-trip every term through snapshot/API/CLI/MCP/web, and migrate the old group value to a one-entry list. Drop a term, admit `*`, widen who may edit, lose a nested member or restore the old snapshot as empty separately: each fails |
