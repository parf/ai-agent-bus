...
| F.13.3 | Service and channel journeys; [Danger Zone built](done/service-danger-zone.md#checks) | [F.13.1](done/web-meanings.md#scope); F.13.2; [built Maintainers-list model](done/maintainers-list.md#checks) | Search by description/address, independently filter availability/reader state, sort and page with state retained, then inspect owner/access/queue information as a read-only visitor. Show a caller-visible User owner's local photo without provider or directory-wide fetching. Preserve invalid ACL and Maintainer lines and never emit glyphs into syntax. Use populated pub/sub and queue fixtures and return to the affected detail. Retain the built Danger Zone, fresh confirmation and resource-specific returns. A protected resource and its owner photo remain hidden. Remove descriptions, hide summaries behind edit permission, accept a Maintainer wildcard, conflate delivery modes, expose a dangerous form on ordinary detail, fetch a provider image from the page or redirect a channel edit to Services separately: each fails |
