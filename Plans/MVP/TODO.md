...
| H.12 | [Activity rewrite](activity-rewrite.md#thesis): a per-record ring of 144 clock-aligned ten-minute slots, saved and restored, gaps for down time; the call counters' minute tick | H.11 | Each [step's acceptance](activity-rewrite.md#steps) has a failing check: slots close at :00/:10 local time, a restart at 15:00 keeps the morning, a down hour is six unobserved slots, the ring wraps after a day, the web draws a fixed day axis with gaps, and "Calls, minute" covers one minute |
