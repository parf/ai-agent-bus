...
| K.10 Add atomic list add/remove operations | Built in 0.7.11 | `core` TestListDeltas, TestADeltaNeedsTheAuthorityAWholeWriteNeeds, TestTheOneSlotDeliverTo, TestConcurrentAddsLoseNothing, TestDeliverToFollowsTheKind, TestAPubSubDeliversToQueuesAndThroughTopics and `api` TestListDeltasOnTheWire fail with each rule removed (the forward-limit mutant crashes the stack rather than failing an assertion); addToSet per owner request 2026-09-22; forwarding through a one-slot route is K.15 |
