...
[decisions](../../docs/decisions.md) indexes every one of them.
...
| 3 | **The runner is a service on the bus**, reached by a call like anything else. No second ssh door; who may deploy on a host is that service's ACL | [runner § reaching the runner](../../docs/08-runner-role.md#reaching-the-runner) |
| 4 | **A service is a checkout; an instance is an env file beside it.** What a service *is* stays externally controlled and holds no secret; what a host *decided* lives apart, so a `git pull` moves no local state | [runner § what an instance is](../../docs/08-runner-role.md#what-an-instance-is) |
| 5 | **Configuration is three layers overlaid, and the more secret one wins.** Precedence and visibility run in opposite directions | [runner § the three env layers](../../docs/08-runner-role.md#the-three-env-layers) |
| 6 | **A declared surface does two jobs**: it makes an upload checkable, and it says whether a service needs an instance at all — so there is no second flag to disagree with | [runner § the three env layers](../../docs/08-runner-role.md#the-three-env-layers) |
| 7 | **The author owns the command line, the host owns whether and how many.** Neither file restates the other | [runner § what an instance is](../../docs/08-runner-role.md#what-an-instance-is) |
| 8 | **Installed, enabled and running are three states with one home each** — systemd's split, and the reason the runner keeps a list rather than walking a tree | [runner § what an instance is](../../docs/08-runner-role.md#what-an-instance-is) |
...
| 10 | **A script holds no credential; a linked service holds its own.** The runner asks for one for a name it owns, like anyone else, and may never mint one | [runner § what the child is told](../../docs/08-runner-role.md#what-the-child-is-told) |
| 11 | **Configuration is write-only** — it goes in and is never handed back. Whoever truly needs the bytes becomes root | [runner § reaching the runner](../../docs/08-runner-role.md#reaching-the-runner) |
| 12 | **The verb is `start`, never `run`** — one word whether you sit in front of it or the runner does it for you | [runner § what the runner does](../../docs/08-runner-role.md#what-the-runner-does) |
| 13 | **A bus that is away is not a service that failed.** The client reconnects; the runner restarts nothing | [runner § where it runs](../../docs/08-runner-role.md#where-it-runs) |
...
Rows moved to [decisions § superseded](../../docs/decisions.md#superseded)
