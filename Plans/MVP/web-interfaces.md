...
Status: individually requested slices are built; F.13.0, the owner’s specification review, completed on 2026-09-18 ([review evidence](done/web-design-owner-review.md#evidence)), F.13.2 completed in 0.5.81 ([shell and recovery](done/web-shell-recovery.md#checks)), and the remaining redesign now depends on F.13.6. [Completed slices](DONE.md#done--mvp) do not close the remaining [acceptance work](TODO.md#web-redesign). Replace the collection of administrative forms with one coherent interface for finding services, understanding delivery, and managing access. Keep Go rendering, make existing information discoverable, and fix misleading states before adding more telemetry. The owner should be able to recognise a session, identify a stalled inbox, and understand the consequence of an administrative action without interpreting API terminology.
