...
Status: individually requested slices are built; F.13.0, the owner’s specification review, completed on 2026-09-18 ([review evidence](done/web-design-owner-review.md#evidence)), F.13.2 completed in 0.5.81 ([shell and recovery](done/web-shell-recovery.md#checks)), and F.13.6 completed in 0.8.26 ([acceptance evidence](done/web-acceptance.md#checks)). [Completed slices](DONE.md#done--mvp) do not close the installed rerun in [F.12](TODO.md#remaining-work). Replace the collection of administrative forms with one coherent interface for finding services, understanding delivery, and managing access. Keep Go rendering, make existing information discoverable, and fix misleading states before adding more telemetry. The owner should be able to recognise a session, identify a stalled inbox, and understand the consequence of an administrative action without interpreting API terminology.
...
| Channels and subscriptions | Built: [topics](../../docs/07-channels.md#the-two-channel-kinds), [subscribers](../../docs/04-messaging.md#subscribers) | Display the record's kind and visible relationships |
...
| [htmx](https://htmx.org/docs/) | HTML fragment updates, progressive enhancement of ordinary links/forms | It is a broad browser runtime; the accepted local selector-submit script does not justify fragment state, history snapshots or script evaluation on private pages | Smallest candidate if the owner later permits partial updates. Keep real URLs/forms; avoid history caching of private pages, script evaluation and third-party assets |
| [Datastar](https://data-star.dev/guide/getting_started), [Go SDK](https://data-star.dev/reference/sdks) | Reactive HTML and server-sent updates from Go | Browser runtime plus stream/reconnect lifecycle; far beyond the accepted selector-submit behavior | Not justified by a bounded registry/administration UI; reconsider for an approved live-monitoring requirement |
