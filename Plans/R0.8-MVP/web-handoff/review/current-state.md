...
| Legacy group delete, **no rendered control** | Handler accepts `action=delete`, `name`, `members` | POST `/groups` → POST `/group`, Remove=true; separate pending API-removal work, not a UI feature to restore | [admin:307][A305], [group policy](../../../../docs/01-identity-and-roles.md#groups) |
