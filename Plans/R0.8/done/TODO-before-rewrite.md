# MVP plan before the documentation rewrite

Historical snapshot; unfinished work is in the active TODO.
...
**Objective**: the MVP as scoped in [stages § MVP](../README.md#scope) —
...
| A.3 | ✅ _done_ — a record carries `--ttl` and `--bound`, a message carries its own `--ttl`, and the record's bounds it: asking for longer than the queue keeps does not get it, and a shorter one on the message still wins ([services § topics](../../../docs/07-channels.md#the-two-channel-kinds)). Neither is a daemon-wide constant any more |
...
- a token alone, with nothing beside it, is served and read as its principal,
  and two tokens are two principals on one socket — putting a name requirement
  back turns *a token with nothing beside it is served* red. "A token survives a
  restart" is **not** a check: the PoC already passes it, though *every
  principal* surviving one is new and is checked;
